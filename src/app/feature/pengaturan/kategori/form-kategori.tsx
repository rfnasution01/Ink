...
export function FormKategori({
  ...
  const handleSubmit = async (e: SyntheticEvent) => {
    ...
    try {
      ...
      const res = id
        ? await axios.patch(`/api/kategori/${id}`, {
            name: name,
            icon: null,
            isDeleted: false,
          })
      ...
      if (res?.status === 201) {
        setMessage(
          id ? "Pengubahan kategori berhasil" : "Penambahan kategori berhasil"
        );
        setTypeMsg("success");

        setTimeout(() => {
          router.push("/pengaturan/kategori");
        }, 1000);
      }

      if (res?.status === 500) {
        setMessage(
          id ? "Pengubahan kategori gagal" : "Penambahan kategori gagal"
        );
        setTypeMsg("warning");
      }
    } catch (error) {
